...
def create_driver_card(driver_data, link):
    ...
    photo_url = tmp_photo_url if tmp_photo_url != None else ""
    print(photo_url + " image fetched")
    card_content = html.Div([
        html.Div([
            html.Img(
                src=photo_url or "https://via.placeholder.com/150x150?text=No+Photo",
        ...
        html.Div([
            ...
            html.P(f"Age at Debut: {abs(datetime.fromisoformat(driver_data['dob']).year - driver_data['start_year'])} years old"),
        ...
        html.Button(
            ...
            style={'margin-top': '20px', 'padding': '10px 20px', 'background-color': '#0066cc', 'color': 'white', 'border': 'none', 'border-radius': '5px', 'cursor': 'pointer'}
        ),
        
        # Hidden div to store driver ID for career timeline
        html.Div(id='driver-id-storage', children=driver_data['driverId'], style={'display': 'none'})
